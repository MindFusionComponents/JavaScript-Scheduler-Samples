    ...
    function handleItemModifying(sender, args) {
        ...
        if (args.action == p.ItemModifyAction.Drag) {
            ...
            if (args.changes == null) {
                ...
                elements.forEach(function (element) { element.style.visibility = 'hidden'; });
            }
            else {
                // clear drag indicator styling
                sender.cells.forEach(function (cell) { cell.bgCell.classList.remove("indicator-drag"); });
                // create drag indicator styling
                var cells = sender.getTimeCells(args.changes.startTime, args.changes.endTime, true);
                cells.forEach(function (cell) { cell.bgCell.classList.add("indicator-drag"); });
            }
        }
        ...
        if (args.action == p.ItemModifyAction.Resize) {
            ...
            if (args.changes == null) {
                ...
                sender.cells.forEach(function (cell) { cell.bgCell.classList.remove("indicator-resize"); });
                // create resize indicator styling
                var cells = sender.getTimeCells(args.changes.startTime, args.changes.endTime, true);
                cells.forEach(function (cell) { cell.bgCell.classList.add("indicator-resize"); });
            }
        }
